...
* **Boiler control** *(optional, off by default)* – the planner can set the
  controller's weekly program, so the heating system stops running its own
  time program against the plan. What it writes is the envelope of all comfort
  periods across all rooms; because the planner knows its schedule in advance,
  there is no offset. Requires the *Heizungsanlagenmanager* add-on. Whoever
  releases the takeover there keeps it released – the planner writes back the
  program it found and does not register again on its own.
